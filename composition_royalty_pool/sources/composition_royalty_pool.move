...
/// `(composition_id, CompositionShare, Currency)` — calling twice with the same Currency
...
fun assert_pool_for_composition<CompositionShare, Currency>(
    ...
    assert!(
        pool.id().to_address() == pool::derived_address<CompositionShare, Currency>(composition_id),
