...
fun test_initialize_pool() {
    ...
    assert!(object::id_to_address(&pool.id()) == pool::derived_address<TEST_SHARE, TEST_CURRENCY>(composition_id));
...
fun test_redeem_aborts_for_wrong_pool() {
    ...
    let mut a_pool: RoyaltyPool<TEST_SHARE, TEST_CURRENCY> =
        scenario.take_shared_by_id(a_pool_id);
    composition_royalty_pool::redeem_and_deposit(&mut b_comp, &b_cap, 100, &mut a_pool);
...
/// per its module doc), and the extension operates on it via `take_shared`
/// in later transactions, not the locally-held `new_for_testing` fixture the
/// other tests here use.
fun test_pool_lifecycle_against_published_and_shared_composition() {
    let mut scenario = test_scenario::begin(ALICE);

    scenario.next_tx(ALICE);
    let (composition, cap) = new_composition_fixture(&mut scenario);
    let composition_id = composition.id();
    ...
    scenario.next_tx(ALICE);
    let mut composition: Composition<TEST_SHARE> = scenario.take_shared_by_id(composition_id);
    let pool = composition_royalty_pool::initialize_pool<TEST_SHARE, TEST_CURRENCY>(
        &mut composition,
        &cap,
    );
    ...
    // Royalty payer's payment lands in the (now shared) composition's funds
    // accumulator — the inbox is available from creation regardless of
    // lifecycle state.
    scenario.next_tx(ALICE);
    balance::create_for_testing<TEST_CURRENCY>(1_000).send_funds(composition_id.to_address());

    // Admin redeems from the accumulator and folds into the pool, taking the
    // shared composition back via `take_shared` exactly as a client PTB would.
    scenario.next_tx(ALICE);
    let mut composition: Composition<TEST_SHARE> = scenario.take_shared_by_id(composition_id);
    let mut pool: RoyaltyPool<TEST_SHARE, TEST_CURRENCY> =
        scenario.take_shared_by_id(pool_id);
